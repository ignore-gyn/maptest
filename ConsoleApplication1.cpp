...
{
private:
	std::pair<unsigned int, std::string> *pArray;
...
void ArrayList::add(std::pair<unsigned int, std::string> vdata)
{
	...
	if (currentIndex >= arraySize) {
		arraySize += GROW_SIZE;		// �{�̃T�C�Y�ɂ���Ȃ�(vector�̎����Ƃ�)O(1), ���̂����O(logN)�ɂȂ�H
...
void VectorData::add(std::pair<unsigned int, std::string> vdata)
{
...
unsigned int VectorData::getSize()
{
	return pArray.size();
}

void VectorData::getAll()
{
	for (std::vector<pVarData>::iterator i = pArray.begin(); i != pArray.end(); ++i) {
		pVarData v = *i;
	}
...
int main()
{
	...
	start = clock();
	for (unsigned int i = 0; i < varData.size(); i++) {		// 23.1mb - 6mb = 17.1mb
	...
	start = clock();
	for (unsigned int i = 0; i < varData.size(); i++) {			// 12.4mb - 6mb = 6.4mb
		arr.add(varData[i]);
		//std::cout << varData[i].first << " = " << varData[i].second << "\n";
	}
	end = clock();
